...
class Mdl_Reports extends CI_Model
{
    ...
    {
        $this->db->select('client_name, client_surname, CONCAT(client_name," ", client_surname) AS client_namesurname, client_vat_id');
        ...
        if ($from_date and $to_date) {
            ...
            $this->db->select("
            (
                SELECT SUM(invoice_item_tax_total) FROM ip_invoice_amounts
                    WHERE ip_invoice_amounts.invoice_id IN
                    (
                        SELECT invoice_id FROM ip_invoices
                            WHERE ip_invoices.client_id = ip_clients.client_id
