...
/**
 * Class Mdl_Incom_Item_Amounts
 */
class Mdl_Incom_Item_Amounts extends CI_Model
{
    /**
     * item_amount_id
     * item_id
    ...
    public function calculate($item_id)
    {
        ...
        $item = $this->mdl_incom_items->get_by_id($item_id);

        $item_price = $item->item_price;
        $item_subtotal = $item->item_quantity * $item_price / (1 + ($item->item_tax_rate_include / 100));
        ...
        if ($this->db->get('ip_incom_item_amounts')->num_rows()) {
            $this->db->where('item_id', $item_id);
            $this->db->update('ip_incom_item_amounts', $db_array);
        } else {
            $this->db->insert('ip_incom_item_amounts', $db_array);
        }
    }
